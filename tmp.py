...
def patchVerification(detector_name, descriptor_name, n, dataset_path, nr_of_iterations=1):
    ...
    for i in range(nr_of_iterations):
        ...
        for folder_idx, folder in enumerate(folders):
            ...
            kp, des = read_keypoints(folder)
            kp_next, des_next = read_keypoints(folders[(folder_idx + 1) % len(folders)])
            ...
            # Select random keypoints from ref image
            ...
            for img_idx, dess in enumerate(des[1:]):
                ...
                tr = transformations[folder_name][img_idx]
                ...
                # TODO: Is this OK?
                dist = kp[img_idx + 1][x_crtano_idx, :][:, [0,1]].T - \
                    imaged_points_normal[[0,1], :]
                ...
                for i in range(imaged_points_normal.shape[1]):
                    # dist_ is the distance from point Hx and every keypoint of image img_idx + 1
                    # TODO: Is this OK?
                    diff = kp[img_idx+1][:,[0,1]].T - imaged_points_normal[[0,1],i].reshape(2,1)
            ...
            for _, dess in enumerate(des_next[1:]):
                bf = cv2.BFMatcher(cv2.NORM_L2)
                matches = bf.match(x_des, dess)
    ...
    # TODO: mAP in case of our patch verification is actually a single AP
    return list_of_APs
    #return list_of_APs, list_of_APs[0]
...
def imageMatching(detector_name, descriptor_name, n, dataset_path, nr_of_iterations=1):
    ...
    for i in range(nr_of_iterations):
        ...
        for id1, folder in enumerate(folders):
            ...
            # TODO: Use a function for reading keypoints + descriptors
...
if __name__ == '__main__':
    ...
    parser_of_args.add_argument('--detector_name', type=str, default='sift',
                                help='name of the detector')
    parser_of_args.add_argument('--descriptor_name', type=str, default='sift',
                                help='name of descriptor')
    parser_of_args.add_argument('--n_keypoints', type=int, default=100,
                                help='number of random keypoints to choose')
    parser_of_args.add_argument('--dataset_path', type=str, default='../../../hpatches-sequences',
                                help='path to hpatches dataset')
    parser_of_args.add_argument('--nr_of_iterations', type=int, default=5,
